...
router.get('/today', async (req, res) => {
  try {
    ...
    const [tasksToday, tasksOverdue, followupsDue, followupsOverdue, promisesToday, promisesBroken, highPriority, recentPayments, newOutstanding, recentlyCleared, overdueBalances] = await Promise.all([
      ...
      ColBalance.find({ ...sf, ...(mine.employeeId ? { salesmanId: mine.employeeId } : {}), status: 'OVERDUE', total: { $gt: 0 } }).sort({ ageDays: -1, total: -1 }).limit(100).lean(),
    ]);
    ...
    const balById = new Map((await ColBalance.find({ dealerId: { $in: ids } }, 'dealerId buckets total status priority nextFollowupAt ageDays oldestPeriod creditDays lastPaymentAt lastPaymentAmount').lean()).map(b => [String(b.dealerId), b]));
    const asObj = b => b instanceof Map ? Object.fromEntries(b) : (b && typeof b === 'object' ? b : {});
    const pend = await pendingByDealer(ids);
    const named = a => a.map(t => { const b = balById.get(String(t.dealerId)); return { ...t, pendingApproval: pend.get(String(t.dealerId))?.amount || 0, pendingRecorded: pend.get(String(t.dealerId))?.recorded || 0, balStatus: b?.status, balPriority: b?.priority, balNextFollowupAt: b?.nextFollowupAt || '', balAgeDays: b?.ageDays ?? null, balOldestPeriod: b?.oldestPeriod || '', balCreditDays: b?.creditDays || 0, balLastPaymentAt: b?.lastPaymentAt || null, balLastPaymentAmount: b?.lastPaymentAmount || 0, came30: pend.get(String(t.dealerId))?.came30 || 0, came30Count: pend.get(String(t.dealerId))?.came30Count || 0, buckets: asObj(t.buckets && Object.keys(asObj(t.buckets)).length ? t.buckets : b?.buckets), balanceTotal: b?.total ?? t.total, dealerName: t.dealerName || names.get(String(t.dealerId))?.name || '', dealerCode: t.dealerCode || names.get(String(t.dealerId))?.code || '', phone: names.get(String(t.dealerId))?.phone || '' }; });
