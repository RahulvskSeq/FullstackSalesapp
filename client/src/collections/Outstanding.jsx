    ...
    <div>
      ...
      <Card style={{ marginBottom: 12 }}>
        <div className="row" style={{ gap: 8, flexWrap: 'wrap' }}>
          ...
          <label className="row" style={{ fontSize: 12, gap: 5, color: 'var(--red)' }} data-tip="Still owing from the oldest month of the latest statement, whatever the status — drops off once that month is cleared"><input type="checkbox" checked={q.overdue === '1'} onChange={e => set({ overdue: e.target.checked ? '1' : '' })} /> due for collection</label>
      ...
      <Card pad={false}>
        {err ? <ErrorBox err={err} onRetry={reload} /> : busy && !data ? <Busy kind="table" rows={8} /> : (
          <Table cols={[
            ...
            { k: 'lastPaymentAt', h: H('lastPaymentAt', 'Came'), r: r => r.lastPaymentAt ? <span data-tip={r.came30 ? `${money(r.came30)} came in the last 30 days (${r.came30Count} payments)` : undefined}><b style={{ color: 'var(--grn)' }}>{money(r.lastPaymentAmount)}</b> · {fmtDate(r.lastPaymentAt)}{r.came30 && r.came30 !== r.lastPaymentAmount ? <span style={{ color: 'var(--t3)' }}> · {money(r.came30)} in 30d</span> : null}</span> : '—' },
          ...
          card={r => <>
            ...
            <div className="col-months" style={{ display: 'grid', gridTemplateColumns: `repeat(${periods.length + 1}, minmax(0,1fr))`, gap: 6, marginBottom: 8 }}>
              {periods.map((p, i) => <KV key={p} k={i === 0 ? <span style={{ color: 'var(--red)' }}>{periodLabel(p)}</span> : periodLabel(p)} v={<span>{r.buckets?.[p] ? (i === 0 ? <OldestPill>{money(r.buckets[p])}</OldestPill> : money(r.buckets[p])) : '–'}</span>} />)}
