      ...
      <div className="modal col-drawer" style={{ maxWidth: 900, width: '100%', maxHeight: '90vh', padding: 18, overflowY: 'auto' }}>
        {busy && !d ? <Busy /> : err ? <ErrorBox err={err} onRetry={reload} /> : (<>
          ...
          <div className="stat-grid col-stats" style={{ gridTemplateColumns: 'repeat(5, 1fr)' }}>
            ...
            <div className="stat-card"><div style={{ fontSize: 10.5, color: 'var(--t3)', fontWeight: 700, textTransform: 'uppercase' }}>Cycles</div><div style={{ fontSize: 19, fontWeight: 800 }}>{num(d.cycles.length)}</div><div style={{ fontSize: 11, color: 'var(--t2)' }}>{d.cycles.filter(c => c.status === 'CLEARED').length} cleared · last came {d.balance?.lastPaymentAmount ? money(d.balance.lastPaymentAmount) + ' on ' : ''}{fmtDate(d.balance?.lastPaymentAt) || '—'}</div></div>
          </div>
          <Tabs value={tab} onChange={setTab} tabs={[{ id: 'overview', label: 'Overview' }, { id: 'timeline', label: 'Timeline', count: tl.data?.items?.length }, { id: 'payments', label: 'Payments', count: d.payments.length }, { id: 'followups', label: 'Follow-ups', count: d.followups.length }, { id: 'promises', label: 'Promises', count: d.promises.length }, { id: 'invoices', label: 'Invoices', count: d.invoices.length }, { id: 'history', label: 'Statements', count: d.snapshots.length }, { id: 'whatsapp', label: 'WhatsApp', count: d.whatsapp.length }]} />
          {tab === 'overview' && <Overview d={d} />}
          {tab === 'timeline' && (tl.busy ? <Busy kind="table" rows={4} /> : <Timeline items={tl.data?.items || []} users={users} />)}
