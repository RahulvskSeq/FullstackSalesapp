  ...
  return(
    <div className="overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal" style={{maxWidth:400}}>
        <div style={{fontSize:16,fontWeight:700,marginBottom:12}}>{action==='delete'?`Delete ${selected.length} dealers?`:action==='status'?`Change Selected User for ${selected.length} dealers`:`Reassign ${selected.length} dealers`}</div>
        {action==='status'&&(<select className="sel inp" value={val} onChange={e=>setVal(e.target.value)} style={{marginBottom:14}}><option value="">Choose Selected User...</option>{['NONE','STAR','KEY ACCOUNT','ACHIEVER','REACTIVE'].map(s=><option key={s}>{s}</option>)}</select>)}
